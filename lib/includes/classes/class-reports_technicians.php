...
class REPORT_TECHNICIANS {
	...
	function generate_form_output_jobs_by_technician( $type, $range ) {
		...
		if ( $type == "date_range" || $type == 'technicians_summary' ) {
			...
			if ( $type !== 'technicians_summary' ) {
				$content .= '<div class="medium-12 cell">';
				$content .= '<label>' . esc_html__( 'Select Technician', 'computer-repair-shop' );
				$content .= wp_dropdown_users( array( 
									'show_option_all' => esc_html__('Select Technician', 'computer-repair-shop'), 
									'name' => 'technician',
									'class' => 'is-visible is-block', 
									'role' => 'technician', 
									'echo' => 0, 'selected' => '' 
								));
				$content .= '</label>';
			...
			if ( $type == 'technicians_summary' ) {
				$content .= '<input type="hidden" name="report_type" value="technicians_summary" />';
			} else {
				$content .= '<input type="hidden" name="report_type" value="jobs_by_technician" />';
			}
	...
	function wc_generate_technician_report( $arges ) {
		...
		$technician 		= ( isset( $arges['technician'] ) && ! empty( $arges['technician'] ) ) ? $arges['technician'] : '';
		$TechfullName		= esc_html__( 'Technician', 'computer-repair-shop' );

		if ( ! empty( $technician ) ) {
			$technician_obj = get_user_by( 'id', $technician );
			$TechfullName = $technician_obj->first_name . ' ' . $technician_obj->last_name;
		}
		if ( $arges["report_type"] == "jobs_by_technician" ) {
			$report_heading = esc_html__( "Jobs Summary By - ", "computer-repair-shop" ) . esc_html( $TechfullName ) ;
		}
	...
	function wc_generate_technicians_summary( $arges ) {
		if ( ! isset( $arges["report_type"] ) || empty( $arges["report_type"] ) ) {
			return esc_html__( "Report is not defined", "computer-repair-shop" );
		}
		$arges['job_status_include']     = ( isset( $_GET['job_status_include'] ) ) ? array_map( 'sanitize_text_field', $_GET['job_status_include'] ) : '';
		...
		if ( $arges["report_type"] == "technicians_summary" ) {
			$report_heading = esc_html__( "Technicians Job Summary", "computer-repair-shop" );
		}
		...
		$content .= '<tr class="heading special_head">
						...
						<td>'.esc_html__("Technician", "computer-repair-shop") . '</td>
		...
		$get_job_data = $this->wc_return_technicians_job_summary( $from_date, $to_date, $arguments );
	...
	function wc_return_technicians_job_summary( $from_date, $end_date, $arges ) {
		...
		if($jobs_query->have_posts()): while($jobs_query->have_posts()):
			...
			$technician = ( ! empty( $technician ) ) ? $technician : 'empty';
			$array_jobs[$technician][] = array(
				'job_id' 		 => $job_id,

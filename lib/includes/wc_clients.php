...
defined( 'ABSPATH' ) || exit;
	...
	function wc_comp_rep_shop_clients() {
...
?>		
		<div class="wrap" id="poststuff">
				<h1 class="wp-heading-inline">
					<?php 
						echo esc_html__( "Manage Clients", "computer-repair-shop" );
					?>
				</h1>
				<a data-open="customerFormReveal" class="page-title-action"><?php echo esc_html__("Add New", "computer-repair-shop"); ?></a>
				<br class="clear" />
				<?php
				...
				?>
				<p><?php echo esc_html__("Display", "computer-repair-shop")." ".esc_html( $display_from )." - ".esc_html( $display_to )." ".esc_html__("From Total", "computer-repair-shop")." ".esc_html( $total_users )." ".esc_html__("Clients.", "computer-repair-shop"); ?></p>
				...
				<tbody data-wp-lists="list:user">
					...
					<?php 
						...
						foreach ( $users_obj->get_results() as $userdata ) {
							...
							$state_province = get_user_meta( $userdata->ID, "billing_state", true );
							$country 		= get_user_meta( $userdata->ID, "billing_country", true );
							...
							$content .= '<td class="username column-username has-row-actions column-primary" data-colname="Username">
											<strong>
												<a href="edit.php?post_type=rep_jobs&job_customer='.$userdata->ID.'">
							...
							$content .= '<td class="company column-tax" data-colname="taxid">';
							$content .= $billing_tax;
							$content .= '</td>';
			
							$content .= '<td class="jobs column-jobs num" data-colname="Jobs">';
							$content .= wc_return_jobs_by_user($userdata->ID, "customer");
							$content .= '</td></tr>';
						}
